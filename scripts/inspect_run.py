...
def print_summary(manifest: RunManifest, phase_filter: Optional[str] = None, json_output: bool = False):
    ...
    if not phases_to_show:
        ...
        for pg in phases_to_show:
            status_icon = "✅" if pg.status == GateStatus.PASS else "❌" if pg.status == GateStatus.FAIL else "⏳"
            print(f"  {status_icon} {pg.phase.upper()}: {pg.status.value.upper()}")
            
            if pg.gates:
                for gate_name, gate_status in pg.gates.items():
                    gate_icon = "✅" if gate_status == GateStatus.PASS else "❌" if gate_status == GateStatus.FAIL else "⏳"
    ...
    if manifest.key_metrics:
        ...
        for metric in manifest.key_metrics:
            ...
            print(f"  {status_icon} {metric.name}: {metric.value} {metric.unit} (threshold: {metric.threshold})")
        print()
