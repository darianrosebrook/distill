...
def validate_tool_call(tool_call: Dict[str, Any], registry: ToolSchemaRegistry) -> List[str]:
    ...
    try:
        ...
        if not is_valid:
            errors.extend([f"Schema validation: {e}" for e in validation_errors] if isinstance(validation_errors, list) else [f"Schema validation: {validation_errors}"])
...
def validate_sample(sample: Dict[str, Any], tokenizer, registry: ToolSchemaRegistry) -> Dict[str, Any]:
    """
