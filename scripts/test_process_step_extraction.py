...
def test_json_extraction():
    ...
    tokenizer = MockTokenizer()
    teacher_text = 'Here is the tool call: {"name": "search", "arguments": {"query": "test", "limit": 10}}'
...
def test_no_tokenizer():
    ...
    targets = extract_process_step_targets(
        teacher_text="Some text with tool calls",
        tokenizer=None,
        tool_names=None
    )
...
def main():
    ...
    for name, targets in results:
        component_count = len([k for k in targets.keys() if k.endswith('_ids') or k.endswith('_mask')])
