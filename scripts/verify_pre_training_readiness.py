...
def check_dataset_validation(
    dataset_path: Path, max_corruption_rate: float = 5.0
) -> Dict[str, Any]:
...
def check_export_contracts() -> Dict[str, Any]:
    ...
    try:
        ...
        if result.returncode != 0:
            results["warnings"].append(
                "Export contract tests failed (may be expected if no model available)"
            )
...
def main():
    ...
    ap.add_argument("--skip-dataset", action="store_true", help="Skip dataset validation")
    ap.add_argument("--skip-export", action="store_true", help="Skip export contract tests")
    ...
    if not args.skip_dataset and args.dataset:
        ...
        if dataset_results["passed"]:
            print(
                f"  PASSED: Dataset validation passed (corruption rate: {dataset_results['corruption_rate']:.2f}%)"
            )
        else:
