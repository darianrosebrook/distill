...
def main():
    ...
    if args.teacher.startswith("hf:"):
        ...
        if tier and tier_limits:
            ...
            print(f"[make_kd_mix] Rate limits: {tier_limits.rpm} RPM, {tier_limits.tpm:,} TPM, "
    ...
    if args.prompts_file:
        ...
        print(f"[make_kd_mix] Generating prompt mix (general={args.general_ratio}, domain={args.domain_ratio}, tool={args.tool_ratio})")
    ...
    print(f"[make_kd_mix] Sampling from teacher (batch_size={args.batch_size}, delay={args.delay}s)...")
    ...
    for i, prompt in enumerate(prompts):
        ...
        if cached_result:
            ...
            cached += 1
            if (i + 1) % 100 == 0:
                print(f"[make_kd_mix] Progress: {i+1}/{len(prompts)} (cached: {cached}, errors: {errors})")
            continue
