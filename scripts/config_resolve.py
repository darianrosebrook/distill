...
def validate_config(config: Dict[str, Any], schema_path: Optional[Path] = None) -> tuple[bool, Optional[str]]:
    """
...
def main():
    ...
    parser.add_argument("--override", type=Path, action="append", help="Override configuration file (can specify multiple)")
    ...
    parser.add_argument("--set", action="append", help="Set config value (e.g., training.batch_size=32)")
    ...
    if args.set:
        for override_str in args.set:
            if '=' not in override_str:
                print(f"Error: Invalid override format: {override_str} (expected key=value)", file=sys.stderr)
                sys.exit(1)
