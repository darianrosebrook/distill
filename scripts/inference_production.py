...
def create_orchestrator(
    ...
    if use_coreml:
        ...
        try:
            ...
            caws_tier = tier_map.get(runtime_config.caws_tier.value if hasattr(runtime_config, 'caws_tier') else runtime_config.get('caws_tier', 'tier_2'), CAWSBudgetTier.TIER_2)
            
            InferenceConfig(
                latent_mode_enabled=runtime_config.latent_mode_enabled if hasattr(runtime_config, 'latent_mode_enabled') else runtime_config.get('latent_mode_enabled', False),
                halt_head_enabled=runtime_config.halt_head_enabled if hasattr(runtime_config, 'halt_head_enabled') else runtime_config.get('halt_head_enabled', False),
                caws_tier=caws_tier,
            )
            
            # Note: CoreML models need special handling - for now, use PyTorch path
            print("[inference_production] WARN: CoreML orchestrator not fully implemented, using PyTorch")
            use_coreml = False
        except Exception as e:
            print(f"[inference_production] WARN: CoreML loading failed: {e}, falling back to PyTorch")
            use_coreml = False
...
def main():
    ...
    ap.add_argument("--tokenizer", default=None, help="Tokenizer path (default: models/student/tokenizer)")
    ...
    if args.track_efficiency:
        try:
            ...
            metrics = EfficiencyMetrics(
                ...
                latent_spans_used=result['output'].count('<bot>') if result['latent_mode_used'] else 0,
