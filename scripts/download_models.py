...
def download_tokenizer(model_id: str, output_dir: Path, verify_vocab_size: int = 32000):
    ...
    try:
        ...
        if vocab_size != verify_vocab_size:
            print(
                f"WARNING: Tokenizer vocab_size={vocab_size} does not match expected {verify_vocab_size}"
            )
...
def main():
    ...
    ap.add_argument(
        "--verify-only", action="store_true", help="Only verify existing tokenizer, do not download"
    )
