...
def main():
    ...
    if args.teacher.startswith("hf:"):
        ...
        client = TeacherClient.from_endpoint(args.teacher, max_retries=5, retry_backoff_factor=2.0)
    ...
    if not args.no_caws_context:
        try:
            ...
            if caws_context:
                caws_context_dict = extract_caws_context_dict(".", spec_id=args.caws_spec_id)
                print(
            ...
            print(f"[make_kd_mix_hardened] WARN: Failed to extract CAWS context: {e}")
                ...
                if teacher_results and not teacher_results[0].get("error"):
                    ...
                    if tokenizer and not args.no_process_supervision:
                        try:
                            ...
                            print(
                                f"[make_kd_mix_hardened] WARN: Failed to extract process-step targets: {e}"
                            )
                    ...
                    if "rate limit" in error_msg.lower() or "429" in error_msg:
                        ...
                        print("[make_kd_mix_hardened] Network error, will retry on next attempt")
