...
def categorize_risk(todo: Dict[str, Any], file_path: str, path_type: str) -> str:
    ...
    blocking_keywords = ["blocking", "critical", "must", "required", "production"]
...
def generate_risk_report(
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(f"- **Total Critical Path TODOs**: {len(critical_todos['training']) + len(critical_todos['conversion'])}\n")
        f.write(f"  - Training path: {len(critical_todos['training'])}\n")
        f.write(f"  - Conversion path: {len(critical_todos['conversion'])}\n")
        f.write(f"- **\"For Now\" Implementations**: {for_now_analysis['total']}\n")
        f.write(f"- **High-Risk \"For Now\" in Critical Paths**: {len(for_now_analysis['high_risk'])}\n\n")
        ...
        for path_type in ["training", "conversion"]:
            f.write(f"### {path_type.capitalize()} Path ({len(critical_todos[path_type])} TODOs)\n\n")
            ...
            for file_path in sorted(by_file.keys()):
                ...
                for todo in sorted(todos_in_file, key=lambda x: x["line"]):
                    f.write(f"**Line {todo['line']}** - Risk: **{todo['risk']}**\n\n")
                    f.write(f"```\n{todo['comment'][:500]}\n```\n\n")
                    f.write(f"- Confidence: {todo['confidence']:.2f}\n")
                    f.write(f"- Context Score: {todo.get('context_score', 0.0):.2f}\n")
        ...
        f.write(f"### Summary\n\n")
        f.write(f"- **Total \"For Now\" Instances**: {for_now_analysis['total']}\n")
        f.write(f"- **In Critical Paths**: {for_now_analysis['by_path']['training'] + for_now_analysis['by_path']['conversion']}\n")
        f.write(f"  - Training: {for_now_analysis['by_path']['training']}\n")
        f.write(f"  - Conversion: {for_now_analysis['by_path']['conversion']}\n")
        ...
        top_files = sorted(for_now_analysis['by_file'].items(), key=lambda x: -x[1])[:10]
        ...
        for path_type in ["training", "conversion"]:
            for todo in critical_todos[path_type]:
                module = todo["file"].split("/")[-1] if "/" in todo["file"] else todo["file"]
        ...
        if critical_count > 0:
            f.write(f"**BLOCKER**: {critical_count} CRITICAL risk TODOs must be resolved before production.\n\n")
        
        if high_count > 0:
            f.write(f"**WARNING**: {high_count} HIGH risk TODOs should be addressed before production.\n\n")
...
def generate_resolution_plan(
    ...
    with open(output_file, 'w') as f:
        ...
        f.write("## Overview\n\n")
        f.write("This plan prioritizes TODO resolution by risk level and production impact.\n\n")
        ...
        if todos_by_risk.get("CRITICAL"):
            ...
            for todo in sorted(todos_by_risk["CRITICAL"], key=lambda x: (x["file"], x["line"])):
                ...
                f.write(f"   - Acceptance Criteria: TODO resolved, tests pass, no production blockers\n\n")
        ...
        if todos_by_risk.get("HIGH"):
            ...
            for file_path in sorted(by_file.keys()):
                ...
                for todo in sorted(todos_in_file, key=lambda x: x["line"]):
                    ...
                    f.write(f"   - Acceptance Criteria: TODO resolved, functionality verified\n\n")
            
            milestone_num += 1
        
        # Milestone 3: "For Now" Implementations
        if for_now_analysis['high_risk']:
            f.write(f"### Milestone {milestone_num}: High-Risk \"For Now\" Implementations\n\n")
            f.write("**Priority**: MEDIUM\n")
            f.write("**Target**: Replace high-risk \"for now\" implementations with proper code\n")
            f.write("**Estimated Effort**: 3-4 weeks\n\n")
            
            f.write(f"**Total High-Risk \"For Now\" Instances**: {len(for_now_analysis['high_risk'])}\n\n")
        ...
        f.write("1. **Review CRITICAL TODOs**: Assess each CRITICAL TODO for actual production impact\n")
        ...
        f.write("2. **Regular Audits**: Schedule quarterly TODO audits to track resolution progress\n")
        f.write("3. **Documentation**: Maintain documentation of temporary implementations and their intended replacements\n")
        f.write("4. **Automated Detection**: Consider adding linting rules to flag new \"for now\" patterns\n\n")
...
def main():
    ...
    generate_resolution_plan(critical_todos, for_now_analysis, resolution_plan_file)
