...
def contains_grounding(text: str, spans: List[List[int]], fields: Dict[str, str]) -> bool:
        ...
        if key in fields:
            ...
            if isinstance(v, (int, float)):
                ...
                try:
                    ...
                    seg_num_alt = (
                        float(seg_digits_alt)
                        if seg_digits_alt and seg_digits_alt != seg_digits
                        else None
                    )
...
def verify_item(
    ...
    if negative_control and integration_grounded:
        ...
        and not integration_grounded
        and integration_spans_bytes
        and tool_result_fields
    ):
...
def check_stratification_backbone(
    ...
    if total < 36:
        ...
        branching_found = any(
            count > 0 for (s, c), count in coverage.items() if c == "branching_error_recovery"
        )
        if not branching_found:
            missing.append(
                {
                    "scenario": "any",
                    "complexity": "branching_error_recovery",
                    "required": 1,
                    "actual": 0,
                }
            )
...
def main():
    ...
    if args.next_registry:
        try:
            ...
            if spec and spec.loader:
                ...
                next_reg = (
                    next_reg_module.ToolSchemaRegistry()
                    if hasattr(next_reg_module, "ToolSchemaRegistry")
                    else None
                )
            if next_reg is None:
                print(
    ...
    total_high_severity_pii = sum(
        r.get("privacy_check", {}).get("high_severity_pii", 0)
        if isinstance(r.get("privacy_check"), dict)
        else 0
        for r in results
    )
    total_medium_severity_pii = sum(
        r.get("privacy_check", {}).get("medium_severity_pii", 0)
        if isinstance(r.get("privacy_check"), dict)
        else 0
        for r in results
    )
    total_contextual_pii = sum(
        r.get("privacy_check", {}).get("contextual_pii", 0)
        if isinstance(r.get("privacy_check"), dict)
        else 0
        for r in results
    )
    ...
    if eligible_results:
        ...
        recalls_lax = [
            r.get("integration_recall_lax", r.get("integration_recall", 0.0))
            for r in eligible_results
        ]
        ...
        total_spans_lax = sum(
            len(item.get("metadata", {}).get("integration_spans_bytes", []))
            for item in eligible_items
        )
        micro_precision_lax = (
            total_grounded_spans_lax / total_spans_lax if total_spans_lax > 0 else 0.0
        )
        ...
        if micro_precision_lax + micro_recall_lax == 0:
            ...
            avg_integration_f1_micro_lax = (
                2
                * micro_precision_lax
                * micro_recall_lax
                / (micro_precision_lax + micro_recall_lax)
            )
        ...
        micro_precision_strict = (
            total_grounded_spans_strict / total_spans_lax if total_spans_lax > 0 else 0.0
        )
        ...
        if micro_precision_strict + micro_recall_strict == 0:
            ...
            avg_integration_f1_micro_strict = (
                2
                * micro_precision_strict
                * micro_recall_strict
                    ...
                    grounding_misses.append(
                        {
                            "line": item_idx + 1,
                            "sample_id": meta.get("sample_id", f"item_{item_idx + 1}"),
                            "tool": meta.get("call_sequence", [{}])[0].get("name")
    ...
    multi_call_items = [
        item for item in items if len(item.get("metadata", {}).get("call_sequence", [])) > 1
    ]
    ...
    num_controls = sum(
        1
        for item in items
        if item.get("metadata", {}).get("expected_behaviour") in {"no_tool", "decline"}
    )
    num_negative_controls = sum(
        1 for item in items if item.get("metadata", {}).get("negative_control", False)
    )
    ...
    summary = {
        ...
        "avg_integration_precision": round(avg_integration_precision, 3)
        if eligible_results
        else None,
        ...
        "avg_integration_precision_macro_lax": round(avg_integration_precision_macro_lax, 3)
        if eligible_results
        else None,
        "avg_integration_recall_macro_lax": round(avg_integration_recall_macro_lax, 3)
        if eligible_results
        else None,
        "avg_integration_f1_macro_strict": round(avg_integration_f1_macro_strict, 3)
        if eligible_results
        else 1.0,
        "avg_integration_precision_macro_strict": round(avg_integration_precision_macro_strict, 3)
        if eligible_results
        else None,
        "avg_integration_recall_macro_strict": round(avg_integration_recall_macro_strict, 3)
        if eligible_results
        else None,
            ...
            generation_plan = {
                ...
                "counts": {
                    "control": sum(
                        1
                        for item in items
                        if item.get("metadata", {}).get("expected_behaviour")
                        not in {"normal", None}
                    ),
                    "adversarial": sum(
                        1 for item in items if "adversarial" in item.get("metadata", {})
                    ),
                    "adversarial_by_type": dict(adversarial_counts),
                    "multilingual": sum(
                        1
                        for item in items
                        if item.get("metadata", {}).get("language")
                        and item.get("metadata", {}).get("language") != "en"
                    ),
                    "long_context": sum(
                        1 for item in items if item.get("metadata", {}).get("long_context")
                    ),
                },
            }
    except Exception:
    ...
    summary["adversarial_taxonomy_ok"] = (
        adversarial_taxonomy_ok if "adversarial_taxonomy_ok" in locals() else True
    )
    summary["adversarial_taxonomy_missing"] = (
        adversarial_taxonomy_missing if "adversarial_taxonomy_missing" in locals() else []
    )
    summary["duplicate_sample_ids"] = (
        duplicate_sample_ids if "duplicate_sample_ids" in locals() else []
    )
    ...
    gates = {
        ...
        "integration_f1_macro_lax": {
            ...
            "min_eligible": args.min_eligible_for_gates,
        },
    ...
    if secondary_tok:
        ...
        if elig_t2:
            summary["token_align_ok_rate_t2"] = (
                round(
                    sum(1 for r in results_t2 if r.get("token_align_ok") is True)
                    / max(1, sum(1 for r in results_t2 if r.get("token_align_ok") is not None)),
                    3,
                )
    ...
    if large_deltas:
        ...
        for t, v in sorted(
            large_deltas.items(),
            key=lambda kv: kv[1].get("delta_lax_minus_strict", 0.0),
            reverse=True,
        ):
            print(
                f"  - {t}: Δ={v.get('delta_lax_minus_strict', 0.0):.3f} (lax={v.get('f1_lax_macro', 0.0):.3f}, strict={v.get('f1_strict_macro', 0.0):.3f}, samples={v.get('sample_count', 0)})"
            )

    # Write report with generation plan
    os.makedirs(
        os.path.dirname(args.report) if os.path.dirname(args.report) else ".", exist_ok=True
    )
    ...
    report_header = {
        ...
        "num_controls": num_controls,
        "num_negative_controls": num_negative_controls
        if "num_negative_controls" in locals()
        else 0,
        "gates": gates,
        "tokenizer_fingerprint": current_tokenizer_fp
        if "current_tokenizer_fp" in locals()
        else None,
        "tool_registry_sha256": current_registry_sha256
        if "current_registry_sha256" in locals()
        else None,
        "integration_span_cap": integration_span_cap,
    ...
    if eligible_results:
        ...
        if num_eligible < args.min_eligible_for_gates:
            ...
            summary["inconclusive_reason"] = (
                f"eligible_items ({num_eligible}) < min_eligible_for_gates ({args.min_eligible_for_gates})"
            )
        # Gate on lax macro-F1 OR misses ≤ allowed (only if we have enough eligible items)
        elif (
            avg_integration_f1_macro_lax < f1_threshold
            and integration_misses_count_lax > allowed_integration_f1_misses
        ):
            print(
        ...
        if secondary_tok and elig_t2 and summary.get("avg_integration_f1_macro_lax_t2") is not None:
            if summary["avg_integration_f1_macro_lax_t2"] < f1_threshold:
                ...
                gates_ok = False
            if (
                summary.get("token_align_ok_rate_t2") is not None
                and summary["token_align_ok_rate_t2"] < 0.98
            ):
                print(
            ...
            low_f1_items = [
                (i, r)
                for i, r in zip(eligible_items, eligible_results)
                if r.get("integration_f1_lax", r.get("integration_f1", 0.0)) < 0.75
            ]
        ...
        # Warn on strict failures (but don't gate)
        if (
            avg_integration_f1_macro_strict < 0.75
            or integration_misses_count_strict > allowed_integration_f1_misses
        ):
            print(
            ...
            low_f1_strict_items = [
                (i, r)
                for i, r in zip(eligible_items, eligible_results)
                if r.get("integration_f1_strict", 0.0) < 0.75
            ]
