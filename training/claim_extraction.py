...
class SimpleClaimExtractor:
    ...
    def _has_factual_structure(self, sentence: str) -> bool:
        ...
        factual_verbs = ['is', 'was', 'are', 'were', 'has', 'have', 'had', 
                        'implements', 'defines', 'creates', 'returns', 'uses']
    ...
    def _decompose_to_atomic(self, sentence: str) -> List[str]:
        ...
        atomic = [p.strip() for p in parts if p.strip() and p.lower() not in ['and', 'or', 'but', ',']]
    ...
    def _compute_confidence(self, statement: str) -> float:
        ...
        pattern_count = sum(1 for p in self.VERIFIABLE_PATTERNS if re.search(p, statement, re.IGNORECASE))
    ...
    def extract_claim_success_rate(self, text: str) -> float:
        ...
        verifiable_count = sum(1 for s in sentences if self._has_verifiable_content(s) and not self._has_unverifiable_content(s))
...
def compute_claim_extraction_metrics(
    student_output: str,
    teacher_output: str,
    extractor: Optional[SimpleClaimExtractor] = None
) -> Dict[str, Any]:
    """
    ...
    success_rate_ratio = student_success_rate / teacher_success_rate if teacher_success_rate > 0 else 0.0
