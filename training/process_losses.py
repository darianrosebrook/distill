...
def tool_selection_loss_from_ids(
    ...
    for i in range(min(batch_size, tool_name_ids.size(0))):
        ...
        # Decode tool name to find where it might appear in sequence
        # We'll search for the tool name in the generated sequence
        # For now, use a simplified approach: compute loss at positions where
        # we expect tool names to appear (e.g., after prompt tokens)
        
        # Get the first token of the tool name as target
        target_token_id = valid_tool_tokens[0].item()
        
        # Find approximate position where tool name should appear
        # This is a heuristic: tool names typically appear after some prompt tokens
        # We'll use a position in the middle-to-end of the sequence where tool calls occur
        seq_len = logits.size(1)
        
        # Try multiple positions and take the best match
        # For now, use a position in the latter half of the sequence
        if seq_len > 10:
            # Check positions in the latter half
            start_pos = seq_len // 2
            best_loss = None
