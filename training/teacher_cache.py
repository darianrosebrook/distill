...
class TeacherCache:
    ...
    def get(
        ...
        if validate_version and entry.teacher_version != self.teacher_version:
            ...
            print(
                f"[TeacherCache] Version mismatch: cached={entry.teacher_version}, current={self.teacher_version}"
            )
            return None
        ...
        if validate_hash:
            ...
            if entry.prompt_hash != expected_hash:
                ...
                print("[TeacherCache] Hash mismatch for prompt")
                return None
    ...
    def clear(self, older_than_days: Optional[int] = None):
        ...
        if older_than_days is None:
            ...
            keys_to_remove = [
                key for key, entry in self._cache.items() if entry.created_at < cutoff_time
            ]
    ...
    def migrate_cache(self, new_version: str):
        ...
        keys_to_remove = [
            key for key, entry in self._cache.items() if entry.teacher_version != new_version
        ]
