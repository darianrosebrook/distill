...
def compute_pairwise_accuracy(predictions: List[int], labels: List[int], 
    ...
    if len(predictions) != len(labels):
        raise ValueError(f"Predictions and labels must have same length: {len(predictions)} != {len(labels)}")
...
def compute_clause_f1(predicted_clauses: List[List[str]], 
    ...
    if len(predicted_clauses) != len(gold_clauses):
        raise ValueError(f"Predicted and gold clauses must have same length: {len(predicted_clauses)} != {len(gold_clauses)}")
    ...
    for clause in all_clauses:
        ...
        clause_f1 = 2 * clause_precision * clause_recall / (clause_precision + clause_recall) if (clause_precision + clause_recall) > 0 else 0.0
    ...
    return ClauseMappingMetrics(
        ...
        precision=precision,
        recall=recall,
        per_clause_metrics=per_clause_metrics
    )
...
def compute_claim_metrics(extracted_claims: List[Dict], 
    ...
    extraction_precision = tp_extraction / (tp_extraction + fp_extraction) if (tp_extraction + fp_extraction) > 0 else 0.0
    extraction_recall = tp_extraction / (tp_extraction + fn_extraction) if (tp_extraction + fn_extraction) > 0 else 0.0
    extraction_f1 = 2 * extraction_precision * extraction_recall / (extraction_precision + extraction_recall) if (extraction_precision + extraction_recall) > 0 else 0.0
    ...
    verification_precision = tp_verification / (tp_verification + fp_verification) if (tp_verification + fp_verification) > 0 else 0.0
    verification_recall = tp_verification / (tp_verification + fn_verification) if (tp_verification + fn_verification) > 0 else 0.0
    verification_f1 = 2 * verification_precision * verification_recall / (verification_precision + verification_recall) if (verification_precision + verification_recall) > 0 else 0.0
    
    # Coverage: fraction of gold claims that were extracted and verified
    total_gold = len(gold_claims)
    extracted_and_verified = sum(1 for claim in gold_claims 
