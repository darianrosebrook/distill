...
def validate_shape_with_model(
    model: torch.nn.Module, shape: int, vocab_size: int, device: str = "cpu"
) -> Dict[str, any]:
    """
    ...
    try:
        ...
        return {"shape": shape, "status": "ok", "error": None, "output_shape": actual_shape}
...
def check_shape_validation_results(
    validation_results: Dict[str, any], require_all: bool = False
) -> Tuple[bool, List[str]]:
    """
    ...
    if not validation_results.get("primary_ok", False):
        errors.append(f"Primary shape {validation_results.get('primary_shape')} validation failed")

    if require_all and not validation_results.get("all_ok", False):
        failed_shapes = [
            r["shape"] for r in validation_results.get("results", []) if r["status"] != "ok"
        ]
        errors.append(f"Shape validation failed for shapes: {failed_shapes}")

    # Collect detailed error messages
    for result in validation_results.get("results", []):
        if result["status"] == "error":
            errors.append(f"Shape {result['shape']}: {result.get('error', 'Unknown error')}")
