...
def compare_prefill_decode_outputs(
    ...
    if pytorch_results["prefill_logits"] is not None and coreml_results["prefill_logits"] is not None:
    ...
    for step in range(num_decode_steps):
        if step < len(pytorch_results["decode_logits"]) and step < len(coreml_results["decode_logits"]):
...
def test_kv_cache_index_advancement():
    ...
    with torch.no_grad():
        ...
        kv_caches = None
        for step in range(5):
            current_token = input_ids[:, -1:] if step == 0 else torch.randint(0, cfg.vocab_size, (1, 1), dtype=torch.int32)
            ...
            assert len(updated_kv_caches) == cfg.n_layers, f"Expected {cfg.n_layers} KV cache layers"
            
            # Verify cache shapes (should grow by 1 token per step)
            for layer_idx, (k_cache, v_cache) in enumerate(updated_kv_caches):
                assert k_cache.shape[2] == v_cache.shape[2], "K and V cache sequence lengths should match"
