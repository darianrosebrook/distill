...
class TestKLDivergence:
    ...
    def test_kl_divergence_with_temperature(self, device):
        ...
        loss_t1 = kl_divergence(
            student_logits, teacher_logits, temperature=1.0)
        loss_t2 = kl_divergence(
            student_logits, teacher_logits, temperature=2.0)
        loss_t05 = kl_divergence(
            student_logits, teacher_logits, temperature=0.5)
    ...
    def test_kl_divergence_reduction_mean(self, device):
        ...
        student_logits = torch.randn(
            2, 5, 100, device=device, requires_grad=True)
    ...
    def test_kl_divergence_reduction_mean_equality_check(self, device):
        ...
        loss_mean = kl_divergence(
            student_logits, teacher_logits, reduction="mean")

        # Test with "sum" reduction (should compute sum)
        loss_sum = kl_divergence(
            student_logits, teacher_logits, reduction="sum")
    ...
    def test_kl_divergence_reduction_none(self, device):
        ...
        seq_len = 5
        student_logits = torch.randn(
            batch_size, seq_len, 100, device=device, requires_grad=True)
    ...
    def test_kl_divergence_temperature_division_verification(self, device):
        ...
        assert not torch.isclose(
            loss_t1, loss_t2, atol=1e-6), "Temperature 1.0 and 2.0 should produce different losses"
        assert not torch.isclose(
            loss_t1, loss_t05, atol=1e-6), "Temperature 1.0 and 0.5 should produce different losses"
        ...
        ), "Temperature scaling should change loss values"
    ...
    def test_kl_divergence_temperature_scaling_effect(self, device):
        ...
        teacher_logits = torch.tensor(
            [[[5.0, 4.0, 3.0, 2.0, 1.0]]], device=device)  # Different distribution
        ...
        assert abs(loss_t1.item() - loss_t2.item()
                   ) > 1e-4, "Different temperatures should produce measurably different losses when division is used"
    ...
    def test_kl_divergence_dimension_check_teacher_logits(self, device):
        ...
        student_logits = torch.randn(
            batch_size, seq_len, vocab_size, device=device, requires_grad=True)
        teacher_logits_3d = torch.randn(
            batch_size, seq_len, vocab_size, device=device)

        # Test with 2D teacher_logits (should NOT be flattened)
        teacher_logits_2d = teacher_logits_3d.view(-1, vocab_size)

        loss_3d = kl_divergence(
            student_logits, teacher_logits_3d, temperature=1.0)
        loss_2d = kl_divergence(
            student_logits, teacher_logits_2d, temperature=1.0)
...
class TestCrossEntropyOnTeacher:
    ...
    def test_cross_entropy_on_teacher_with_ignore_index(self, device):
        ...
        loss = cross_entropy_on_teacher(
            student_logits, teacher_targets, ignore_index=-100)
...
class TestToolNameLoss:
    ...
    def test_tool_name_loss_basic(self, device):
        ...
        student_logits = torch.randn(
            batch_size, seq_len, vocab_size, device=device, requires_grad=True)
        tool_name_ids = torch.randint(
            0, vocab_size, (batch_size, tool_len), device=device)
    ...
    def test_tool_name_loss_with_mask(self, device):
        ...
        tool_name_mask = torch.tensor(
            [[1, 1, 0, 0, 0], [1, 1, 1, 0, 0]], device=device)
...
class TestJSONArgumentLoss:
    ...
    def test_json_argument_loss_basic(self, device):
        ...
        student_logits = torch.randn(
            batch_size, seq_len, vocab_size, device=device, requires_grad=True)
        gold_json_text_ids = torch.randint(
            0, vocab_size, (batch_size, json_len), device=device)
        mask_valid_json_tokens = torch.ones(
            batch_size, json_len, device=device)
...
class TestIntegrationCopyLoss:
    ...
    def test_integration_copy_loss_basic(self, device):
        ...
        student_logits = torch.randn(
            batch_size, seq_len, vocab_size, device=device, requires_grad=True)
        tool_result_fields = torch.randint(
            0, vocab_size, (batch_size, int_len), device=device)
    ...
    def test_integration_copy_loss_with_mask(self, device):
        ...
        integration_mask = torch.tensor(
            [[1, 1, 0, 0, 1, 1], [1, 0, 0, 1, 1, 1]], device=device)
...
class TestIntermediateLayerLoss:
    ...
    def test_intermediate_layer_loss_basic(self, device):
        """Test intermediate layer loss with basic aligned pairs."""
        batch_size = 2
        seq_len = 10
        d_model = 128  # Same dimension for both student and teacher

        # Create aligned pairs: (student_layer, teacher_layer) -> (student_hidden, teacher_hidden)
        # The function expects tensors that are already in the same dimensional space
        aligned_pairs = {
            (6, 6): (
        ...
        result = intermediate_layer_loss(aligned_pairs)
    ...
    def test_intermediate_layer_loss_with_projection(self, device):
        """Test intermediate layer loss with projected tensors."""
        batch_size = 2
        seq_len = 10
        d_model = 128  # Same dimension for both student and teacher

        # Create aligned pairs where tensors are already in the same dimensional space
        # (simulating the result of projection)
        aligned_pairs = {
            (0, 0): (
                torch.randn(batch_size, seq_len, d_model,
                            device=device, requires_grad=True),
    ...
    def test_intermediate_layer_loss_sequence_mismatch(self, device):
        """Test intermediate layer loss with multiple layer pairs."""
        batch_size = 2
        seq_len = 10
        d_model = 128

        # Test with multiple aligned pairs
        aligned_pairs = {
            (0, 0): (
                torch.randn(batch_size, seq_len, d_model,
                            device=device, requires_grad=True),
        ...
        # Should compute loss over multiple pairs
        assert isinstance(result, dict)
        assert "total" in result
        loss = result["total"]
        assert isinstance(loss, torch.Tensor)
    ...
    def test_intermediate_layer_loss_empty_mapping(self, device):
        ...
        # Create aligned pairs dict as expected by the current API (empty)
        aligned_pairs = {}
    ...
    def test_intermediate_layer_loss_boundary_check_gte(self, device):
        ...
        # student_idx >= len, should be skipped
        layer_mapping_boundary = {2: 0}

        # Test with teacher_idx at boundary (should be skipped)
        # teacher_idx = 3, len(teacher_hidden_states) = 3, so 3 >= 3 should skip
        # teacher_idx >= len, should be skipped
        layer_mapping_boundary2 = {0: 3}
        ...
        # Create aligned pairs dict as expected by the current API
        aligned_pairs_boundary1 = {}
        for student_layer, teacher_layer in layer_mapping_boundary.items():
            if student_layer < len(student_hidden_states) and teacher_layer < len(teacher_hidden_states):
                aligned_pairs_boundary1[(student_layer, teacher_layer)] = (
                    student_hidden_states[student_layer],
                    teacher_hidden_states[teacher_layer],
                )
        result_boundary1 = intermediate_layer_loss(aligned_pairs_boundary1)

        aligned_pairs_boundary2 = {}
        for student_layer, teacher_layer in layer_mapping_boundary2.items():
            if student_layer < len(student_hidden_states) and teacher_layer < len(teacher_hidden_states):
                aligned_pairs_boundary2[(student_layer, teacher_layer)] = (
                    student_hidden_states[student_layer],
                    teacher_hidden_states[teacher_layer],
                )
        result_boundary2 = intermediate_layer_loss(aligned_pairs_boundary2)

        # Valid case should return non-zero loss
        aligned_pairs_valid = {}
        for student_layer, teacher_layer in layer_mapping_valid.items():
            if student_layer < len(student_hidden_states) and teacher_layer < len(teacher_hidden_states):
                aligned_pairs_valid[(student_layer, teacher_layer)] = (
                    student_hidden_states[student_layer],
                    teacher_hidden_states[teacher_layer],
                )
        result_valid = intermediate_layer_loss(aligned_pairs_valid)

        # Boundary cases should return zero (skipped)
        assert result_boundary1["total"].item(
        ) == 0.0, "student_idx >= len should be skipped"
        assert result_boundary2["total"].item(
        ) == 0.0, "teacher_idx >= len should be skipped"

        # Valid case should return non-zero loss
        assert result_valid["total"].item(
        ) > 0, "Valid indices should produce non-zero loss"
    ...
    def test_intermediate_layer_loss_boundary_equality_case(self, device):
        ...
        # student_idx == len, should be skipped with >=
        layer_mapping_equality = {2: 0}

        # Test with teacher_idx exactly equal to length (should be skipped with >=, not skipped with ==)
        # teacher_idx == len, should be skipped with >=
        layer_mapping_equality2 = {0: 3}

        # Boundary cases should return zero (skipped with >= check)
        # Create aligned pairs dict as expected by the current API
        aligned_pairs_equality1 = {}
        for student_layer, teacher_layer in layer_mapping_equality.items():
            if student_layer < len(student_hidden_states) and teacher_layer < len(teacher_hidden_states):
                aligned_pairs_equality1[(student_layer, teacher_layer)] = (
                    student_hidden_states[student_layer],
                    teacher_hidden_states[teacher_layer],
                )
        result_equality1 = intermediate_layer_loss(aligned_pairs_equality1)

        aligned_pairs_equality2 = {}
        for student_layer, teacher_layer in layer_mapping_equality2.items():
            if student_layer < len(student_hidden_states) and teacher_layer < len(teacher_hidden_states):
                aligned_pairs_equality2[(student_layer, teacher_layer)] = (
                    student_hidden_states[student_layer],
                    teacher_hidden_states[teacher_layer],
                )
        result_equality2 = intermediate_layer_loss(aligned_pairs_equality2)
        ...
        assert result_equality1["total"].item(
        ) == 0.0, "student_idx == len should be skipped with >= check"
        assert result_equality2["total"].item(
        ) == 0.0, "teacher_idx == len should be skipped with >= check"
    ...
    def test_code_mode_preference_loss_tool_count_gte_min_tools(self, device):
        ...
        batch_meta_equality = [
            {"tool_count": 2, "intermediate_sizes": [],
                "pii_tags_present": False},  # tool_count == min_tools (2)
        ]

        # Test with tool_count > min_tools (should be eligible)
        batch_meta_above = [
            {"tool_count": 3, "intermediate_sizes": [],
                "pii_tags_present": False},  # tool_count > min_tools (2)
        ]

        # Test with tool_count < min_tools (should NOT be eligible)
        batch_meta_below = [
            {"tool_count": 1, "intermediate_sizes": [],
                "pii_tags_present": False},  # tool_count < min_tools (2)
        ]
        ...
        loss_equality = loss_fn(
            student_logits, span_targets=span_targets, batch_meta=batch_meta_equality)

        # Above case should be eligible
        loss_above = loss_fn(
            student_logits, span_targets=span_targets, batch_meta=batch_meta_above)

        # Below case should NOT be eligible (zero loss)
        loss_below = loss_fn(
            student_logits, span_targets=span_targets, batch_meta=batch_meta_below)
    ...
    def test_code_mode_preference_loss_tool_count_gte_boundary_check(self, device):
        ...
        batch_meta_equality = [
            {"tool_count": 2, "intermediate_sizes": [],
                "pii_tags_present": False},  # tool_count == 2 (min_tools)
        ]

        # Test with tool_count > min_tools (should be eligible)
        batch_meta_above = [
            {"tool_count": 3, "intermediate_sizes": [],
                "pii_tags_present": False},  # tool_count > 2
        ]
...
class TestCreateProjectionLayers:
    ...
    def test_create_projection_layers_output_shape(self, device):
        ...
        batch_size = 2
        seq_len = 10
        student_h = torch.randn(batch_size, seq_len,
                                student_d_model, device=device)
    ...
    def test_create_projection_layers_none_append(self, device):
        ...
        assert projection_layers[
            0] is not None, "Layer 0 should have projection (in mapping)"
        assert projection_layers[
            1] is None, "Layer 1 should be None (not in mapping, None append verified)"
        assert projection_layers[
            2] is not None, "Layer 2 should have projection (in mapping)"
...
class TestLengthAwareKDLoss:
    ...
    def test_length_aware_kd_loss_basic(self, device):
        ...
        student_attn_mask = torch.ones(
            batch_size, student_seq_len, device=device, requires_grad=True)
        teacher_attn_mask = torch.ones(
            batch_size, teacher_seq_len, device=device)
    ...
    def test_length_aware_kd_loss_with_hinge(self, device):
        ...
        required_fields_present = torch.tensor(
            [False, False], device=device)  # Missing fields
    ...
    def test_length_aware_kd_loss_rel_excess_gt_hinge(self, device):
        ...
        student_attn_mask = torch.ones(
            batch_size, seq_len + 5, device=device)  # 25 tokens
        teacher_attn_mask = torch.ones(
            batch_size, seq_len, device=device)  # 20 tokens
        # rel_excess = (25 - 20) / 20 = 0.25 (above hinge of 0.15)
        required_fields_present = torch.zeros(
            batch_size, dtype=torch.bool, device=device)  # Missing fields
        ...
        student_attn_mask_at_hinge = torch.ones(
            batch_size, 23, device=device)  # Exactly 15% longer
        teacher_attn_mask_at_hinge = torch.ones(
        ...
        # Test with rel_excess < hinge (should NOT penalize)
        student_attn_mask_below_hinge = torch.ones(
            batch_size, seq_len + 1, device=device)  # 21 tokens (5% longer)
        teacher_attn_mask_below_hinge = torch.ones(
            batch_size, seq_len, device=device)
...
class TestEarlyToolCallLoss:
    ...
    def test_early_tool_call_loss_with_teacher_prefix(self, device):
        ...
        teacher_prefix_ids = torch.randint(
    ...
    def test_early_tool_call_loss_teacher_prefix_numel_gt_zero(self, device):
        ...
        teacher_prefix_ids_nonzero = torch.randint(
            0, vocab_size, (batch_size, 10), device=device)
        ...
        teacher_prefix_ids_zero = torch.zeros(
            (batch_size, 0), dtype=torch.long, device=device)  # Empty tensor
    ...
    def test_early_tool_call_loss_without_teacher_prefix_json_prior(self, device):
        ...
        mock_tokenizer.convert_tokens_to_ids = Mock(
            side_effect=mock_convert_tokens_to_ids)
        ...
        assert "early_tool.frac_target_available" in diags
        # No teacher prefix
        assert diags["early_tool.frac_target_available"] == 0.0
    ...
    def test_early_tool_call_loss_disabled(self, device, mock_tokenizer):
        ...
        tool_should_be_used = torch.tensor(
            [False, False], device=device)  # No tools needed
...
class TestCombinedKDLoss:
    ...
    def test_combined_kd_loss_basic(self, device):
        ...
        teacher_targets = torch.randint(
            0, vocab_size, (batch_size, seq_len), device=device)
        ground_truth_targets = torch.randint(
            0, vocab_size, (batch_size, seq_len), device=device)
    ...
    def test_combined_kd_loss_json_loss_addition(self, device):
        ...
        assert loss_dict_with_json["json_args"].item(
        ) >= 0, "json_loss should be non-negative"
        ...
        0.15 * loss_dict_with_json["json_args"].item()
        actual_increase = loss_dict_with_json["total"].item(
        ) - loss_dict_no_json["total"].item()
    ...
    def test_combined_kd_loss_json_loss_weight_multiplication(self, device):
        ...
        json_loss = loss_dict_w01["json_args"].item()
        ...
        actual_difference = loss_dict_w02["total"].item(
        ) - loss_dict_w01["total"].item()
        expected_difference_with_mult = (
            0.2 - 0.1) * json_loss  # 0.1 * json_loss
    ...
    def test_combined_kd_loss_ground_truth_targets_none_check(self, device):
        ...
        loss_dict_provided = combined_kd_loss(
            ...
            teacher_logits,
            teacher_targets,
            # Provided - should compute CE on ground truth
            ground_truth_targets=ground_truth_targets,
            kl_weight=0.5,
            ce_teacher_weight=0.3,
            ce_ground_truth_weight=0.2,
...
class TestCodeModePreferenceLoss:
    ...
    def test_code_mode_preference_loss_basic(self, device):
        ...
        eligibility_rules = {"min_tools": 2,
                             "min_intermediate_chars": 10000, "pii_patterns": []}
        reward = {"prefer_ts_api_over_direct_tool": True,
                  "penalize_tool_result_roundtrip": False}
        vocab_ids = {"import": 100, "from": 101}
    ...
    def test_code_mode_preference_loss_forward_with_eligibility(self, device):
        ...
        batch_meta = [
            {"tool_count": 3, "intermediate_sizes": [],
                "pii_tags_present": False},  # Eligible
            {"tool_count": 1, "intermediate_sizes": [],
                "pii_tags_present": False},  # Not eligible
        ]

        # Span targets: TS API spans and direct tool spans
        span_targets = [
            {
                # TS API spans for first sample
                "ts_mode_spans": [(5, 10), (15, 18)],
        ...
        loss = loss_fn(student_logits, span_targets=span_targets,
    ...
    def test_code_mode_preference_loss_forward_no_span_targets(self, device):
        ...
        # No span targets
        loss = loss_fn(student_logits, span_targets=None,
                       batch_meta=batch_meta)
    ...
    def test_code_mode_preference_loss_eligibility_intermediate_chars(self, device):
        ...
        batch_meta = [
            {"tool_count": 1, "intermediate_sizes": [
                5000, 12000], "pii_tags_present": False},  # Eligible (max >= 10000)
            {"tool_count": 1, "intermediate_sizes": [
                5000], "pii_tags_present": False},  # Not eligible
        ]
    ...
    def test_code_mode_preference_loss_eligibility_pii_tags(self, device):
        ...
        batch_meta = [
            {"tool_count": 1, "intermediate_sizes": [],
                "pii_tags_present": True},  # Eligible
            {"tool_count": 1, "intermediate_sizes": [],
                "pii_tags_present": False},  # Not eligible
        ]
    ...
    def test_code_mode_preference_loss_span_targets_none_handling(self, device):
        ...
        batch_meta = [
            {"tool_count": 3, "intermediate_sizes": [
                15000], "pii_tags_present": False},
            {"tool_count": 2, "intermediate_sizes": [
                12000], "pii_tags_present": False},
        ]

        # Test with span_targets=None explicitly (default value)
        loss_none = loss_fn(
            student_logits, span_targets=None, batch_meta=batch_meta)

        # Test with span_targets as empty list
        loss_empty = loss_fn(
            student_logits, span_targets=[], batch_meta=batch_meta)
    ...
    def test_code_mode_preference_loss_weight_multiplication_verification(self, device):
        ...
        span_targets = [
            {"ts_mode_spans": [], "direct_tool_spans": [
                (5, 10)]},  # Has direct tool spans
        ]

        # Compute losses with different weights
        loss_light = loss_fn_light(
            student_logits, span_targets=span_targets, batch_meta=batch_meta)
        loss_heavy = loss_fn_heavy(
            student_logits, span_targets=span_targets, batch_meta=batch_meta)
    ...
    def test_code_mode_preference_loss_vocab_ids_none_handling(self, device):
        ...
        loss_none = loss_fn_none(
            student_logits, span_targets=span_targets, batch_meta=batch_meta)
        loss_provided = loss_fn_provided(
            student_logits, span_targets=span_targets, batch_meta=batch_meta)
        ...
        assert loss_provided.item() >= 0 or loss_provided.item() < 0
    ...
    def test_code_mode_preference_loss_eligibility_intermediate_chars_true_check(self, device):
        ...
        batch_meta_eligible = [
            {"tool_count": 1, "intermediate_sizes": [
                15000], "pii_tags_present": False},  # Eligible (max >= 10000)
            {"tool_count": 1, "intermediate_sizes": [
                5000], "pii_tags_present": False},  # Not eligible
        ]
        ...
        loss_eligible = loss_fn(
            student_logits, span_targets=span_targets, batch_meta=batch_meta_eligible)
    ...
    def test_code_mode_preference_loss_intermediate_sizes_length_check(self, device):
        ...
        batch_meta_empty = [
            {"tool_count": 1, "intermediate_sizes": [],
                "pii_tags_present": False},  # Empty list, not eligible
            {"tool_count": 1, "intermediate_sizes": [],
                "pii_tags_present": False},  # Empty list, not eligible
        ]
        ...
        # tool_count < min_tools (1 < 2), so only intermediate_sizes path can make it eligible
        batch_meta_nonempty = [
            # Non-empty, eligible (max >= 10000)
            {"tool_count": 1, "intermediate_sizes": [
                15000], "pii_tags_present": False},
            # Non-empty, not eligible (max < 10000)
            {"tool_count": 1, "intermediate_sizes": [
                5000], "pii_tags_present": False},
        ]
        ...
        loss_empty = loss_fn(
            student_logits, span_targets=span_targets, batch_meta=batch_meta_empty)
        loss_nonempty = loss_fn(
            student_logits, span_targets=span_targets, batch_meta=batch_meta_nonempty)
    ...
    def test_code_mode_preference_loss_num_eligible_gt_zero_check(self, device):
        ...
        batch_meta_eligible = [
            {"tool_count": 3, "intermediate_sizes": [],
                "pii_tags_present": False},  # Eligible (tool_count >= 2)
            {"tool_count": 3, "intermediate_sizes": [],
                "pii_tags_present": False},  # Eligible (tool_count >= 2)
        ]
        ...
        loss_not_eligible = loss_fn(
            student_logits, span_targets=span_targets, batch_meta=batch_meta_not_eligible)
    ...
    def test_code_mode_preference_loss_prefer_ts_api_false_branch(self, device):
        ...
        loss_fn = CodeModePreferenceLoss(
            eligibility_rules={
                "min_tools": 2, "min_intermediate_chars": 10000, "pii_patterns": []},
            reward={
                # This should skip the if branch at line 830
                "prefer_ts_api_over_direct_tool": False,
                "penalize_tool_result_roundtrip": True,
            },
    ...
    def test_code_mode_preference_loss_penalize_tool_result_roundtrip_false_branch(self, device):
        ...
        loss_fn = CodeModePreferenceLoss(
            ...
            reward={
                ...
                # This should skip the if branch at line 840
                "penalize_tool_result_roundtrip": False,
            },
            weights={"pos": 0.5, "neg": 0.3},
        )
    ...
    def test_code_mode_preference_loss_penalize_tool_result_roundtrip_true_check(self, device):
        ...
        reward_true = {"prefer_ts_api_over_direct_tool": True,
                       "penalize_tool_result_roundtrip": True}
        # Test with False (explicit)
        reward_false = {"prefer_ts_api_over_direct_tool": True,
                        "penalize_tool_result_roundtrip": False}
        vocab_ids = {"import": 100, "from": 101}
        ...
        span_targets = [
            {"ts_mode_spans": [(5, 10)], "direct_tool_spans": [
                (0, 3)]},  # Has direct tool spans
        ]

        loss_true = loss_fn_true(
            student_logits, span_targets=span_targets, batch_meta=batch_meta)
        loss_false = loss_fn_false(
            student_logits, span_targets=span_targets, batch_meta=batch_meta)
    ...
    def test_code_mode_preference_loss_span_boundary_check_end_lte(self, device):
        ...
        span_targets_boundary = [
            # end == seq_len (boundary)
            {"ts_mode_spans": [(5, 20)], "direct_tool_spans": []},
        ]

        # Test with end less than seq_len (valid case)
        span_targets_valid = [
            # end < seq_len (valid)
            {"ts_mode_spans": [(5, 19)], "direct_tool_spans": []},
        ]

        # Test with end greater than seq_len (out of bounds, should be skipped)
        span_targets_oob = [
            # end > seq_len (out of bounds)
            {"ts_mode_spans": [(5, 21)], "direct_tool_spans": []},
        ]

        # Boundary case should work (end == seq_len is valid with <= check)
        loss_boundary = loss_fn(
            student_logits, span_targets=span_targets_boundary, batch_meta=batch_meta)

        # Valid case should work
        loss_valid = loss_fn(
            student_logits, span_targets=span_targets_valid, batch_meta=batch_meta)

        # Out of bounds case should skip the span (end > seq_len)
        loss_oob = loss_fn(
            student_logits, span_targets=span_targets_oob, batch_meta=batch_meta)
        ...
        # With ==: end < seq_len would be skipped (wrong behavior)
        assert isinstance(
            loss_boundary, torch.Tensor), "Boundary case (end == seq_len) should work with <= check"
        assert isinstance(
            loss_valid, torch.Tensor), "Valid case (end < seq_len) should work"
        assert isinstance(
            loss_oob, torch.Tensor), "Out of bounds case should be handled"
    ...
    def test_code_mode_preference_loss_span_boundary_and_check(self, device):
        ...
        span_targets_both_true = [
            # Both start < size(1) and end <= size(1)
            {"ts_mode_spans": [(5, 10)], "direct_tool_spans": []},
        ]

        # Test with first condition false, second true (should NOT process span)
        span_targets_first_false = [
            # start >= size(1), but end might be <= size(1)
            {"ts_mode_spans": [(25, 30)], "direct_tool_spans": []},
        ]

        # Test with first condition true, second false (should NOT process span)
        span_targets_second_false = [
            # start < size(1), but end > size(1)
            {"ts_mode_spans": [(5, 25)], "direct_tool_spans": []},
        ]

        loss_both_true = loss_fn(
            student_logits, span_targets=span_targets_both_true, batch_meta=batch_meta)
        loss_first_false = loss_fn(
            student_logits, span_targets=span_targets_first_false, batch_meta=batch_meta)
        loss_second_false = loss_fn(
            student_logits, span_targets=span_targets_second_false, batch_meta=batch_meta)
    ...
    def test_code_mode_preference_loss_span_boundary_check_start_lt(self, device):
        ...
        span_targets_start_oob = [
            # start == seq_len (out of bounds)
            {"ts_mode_spans": [(20, 21)], "direct_tool_spans": []},
        ]

        # Test with start less than seq_len (valid case)
        span_targets_start_valid = [
            # start < seq_len (valid)
            {"ts_mode_spans": [(19, 20)], "direct_tool_spans": []},
        ]

        # Out of bounds case should skip the span (start >= seq_len)
        loss_start_oob = loss_fn(
            student_logits, span_targets=span_targets_start_oob, batch_meta=batch_meta)

        # Valid case should work
        loss_start_valid = loss_fn(
            student_logits, span_targets=span_targets_start_valid, batch_meta=batch_meta)
        ...
        assert isinstance(
            loss_start_oob, torch.Tensor), "Out of bounds case (start >= seq_len) should be handled"
        assert isinstance(
            loss_start_valid, torch.Tensor), "Valid case (start < seq_len) should work"
...
class TestCAWSComplianceLoss:
    ...
    def test_caws_compliance_loss_latent_span_count_boundary(self, device):
        ...
        student_output_long_high_latent = "A" * 250 + \
            " <bot>span1</bot> <bot>span2</bot> <bot>span3</bot>"

        loss_short_high = caws_compliance_loss(
            student_output_short_high_latent, teacher_output)
        loss_short_low = caws_compliance_loss(
            student_output_short_low_latent, teacher_output)
        loss_long_high = caws_compliance_loss(
            student_output_long_high_latent, teacher_output)
    ...
    def test_caws_compliance_loss_output_length_boundary_lt(self, device):
        ...
        student_output_boundary = "A" * 200 + \
            " <bot>span1</bot> <bot>span2</bot> <bot>span3</bot>"
        teacher_output = "Teacher output"

        # Test with output_length < 200 (should add penalty if latent_span_count > 2)
        student_output_short = "A" * 199 + \
            " <bot>span1</bot> <bot>span2</bot> <bot>span3</bot>"

        # Test with output_length > 200 (should NOT add penalty)
        student_output_long = "A" * 201 + \
            " <bot>span1</bot> <bot>span2</bot> <bot>span3</bot>"

        loss_boundary = caws_compliance_loss(
            student_output_boundary, teacher_output)
    ...
    def test_caws_compliance_loss_tool_usage_in_check(self, device):
        ...
        loss_with_tool = caws_compliance_loss(
            student_output_with_tool, teacher_output)
        loss_no_tool = caws_compliance_loss(
            student_output_no_tool, teacher_output)
    ...
    def test_caws_compliance_loss_unsupported_claims_increment(self, device):
        ...
        class MockClaimExtractor:
            def extract_claims(self, text):
                if "student" in text.lower():
                    ...
                    # 2 teacher claims (only claim1 and claim2 supported)
        ...
        loss_with_extractor = caws_compliance_loss(
            student_output, teacher_output, claim_extractor=claim_extractor)

        # Test without claim extractor (should not penalize unsupported claims)
        loss_without_extractor = caws_compliance_loss(
            student_output, teacher_output, claim_extractor=None)
    ...
    def test_claim_supported_by_teacher_substring_or_check(self, device):
        ...
        loss1 = caws_compliance_loss(
            student_output1, teacher_output1, claim_extractor=claim_extractor)
        loss2 = caws_compliance_loss(
            student_output2, teacher_output2, claim_extractor=claim_extractor)
        loss3 = caws_compliance_loss(
            student_output3, teacher_output3, claim_extractor=claim_extractor)
    ...
    def test_claim_supported_by_teacher_if_statement_true_check(self, device):
        ...
        class MockClaimExtractor:
            def extract_claims(self, text):
                ...
                if "student" in text.lower():
                    ...
                    # Empty teacher claims (triggers if not teacher_claims: return False)
                    return []
        ...
        loss_empty = caws_compliance_loss(
            student_output, teacher_output, claim_extractor=claim_extractor)
        ...
        loss_nonempty = caws_compliance_loss(
            student_output, teacher_output, claim_extractor=claim_extractor_nonempty)
    ...
    def test_caws_compliance_loss_if_statement_false_check(self, device):
        ...
        """
        # Test with output_length < 200 and latent_span_count > 2 (should add penalty)
        student_output_short_high = "A" * 199 + \
            " <bot>span1</bot> <bot>span2</bot> <bot>span3</bot>"
        teacher_output = "Teacher output"
        ...
        loss_short_high = caws_compliance_loss(
            student_output_short_high, teacher_output)
        loss_long = caws_compliance_loss(student_output_long, teacher_output)
        loss_short_low = caws_compliance_loss(
            student_output_short_low, teacher_output)
        ...
        assert loss_short_high.item(
        ) >= 0, "Short output with high latent count should have penalty (if statement verified)"
    ...
    def test_caws_compliance_loss_latent_span_count_boundary_check(self, device):
        ...
        penalty_count_2 = _evaluate_feature_usage_compliance(
            student_output_count_2)
        penalty_count_3 = _evaluate_feature_usage_compliance(
            student_output_count_3)
        penalty_count_1 = _evaluate_feature_usage_compliance(
            student_output_count_1)
    ...
    def test_caws_compliance_loss_output_length_lt_check(self, device):
        ...
        penalty_short = _evaluate_quality_compliance(
            student_output_short, teacher_output)
        penalty_exact = _evaluate_quality_compliance(
            student_output_exact, teacher_output)
        penalty_long = _evaluate_quality_compliance(
            student_output_long, teacher_output)
        ...
        assert len(student_output_short.strip()
                   ) == 49, "Short output should be 49 chars"
        assert len(student_output_exact.strip()
                   ) == 50, "Exact output should be 50 chars"
        assert len(student_output_long.strip()
                   ) == 100, "Long output should be 100 chars"
    ...
    def test_caws_compliance_loss_step_patterns_increment(self, device):
        ...
        budget_penalty_many_steps = _evaluate_budget_compliance(
            student_output_many_steps)
        budget_penalty_few_steps = _evaluate_budget_compliance(
            student_output_few_steps)
        ...
        clean_penalty_many = _evaluate_budget_compliance(
            clean_output_many_steps)
    ...
    def test_caws_compliance_loss_code_patterns_penalty_increment(self, device):
        ...
        loss_code_no_tool = caws_compliance_loss(
            student_output_code_no_tool, teacher_output)
        loss_code_with_tool = caws_compliance_loss(
            student_output_code_with_tool, teacher_output)
        loss_no_code = caws_compliance_loss(
            student_output_no_code, teacher_output)
        ...
        assert loss_code_no_tool.item(
        ) >= 0, "Code patterns without tool usage should have penalty (increment verified)"
        assert loss_code_with_tool.item(
        ) >= 0, "Code patterns with tool usage should have lower or zero penalty"
        ...
        class MockClaimExtractor:
            def extract_claims(self, text):
                ...
                if "student" in text.lower():
                    ...
                    # Teacher claim containing student claim
                    return ["teacher claim with student claim in it"]
...
class TestCAWSStructureLoss:
    ...
    def test_caws_structure_loss_equality_case_gte(self, device):
        ...
        score = 0.7
        loss_equal = caws_structure_loss(
            teacher_score=score, student_score=score)

        # Test with student_score > teacher_score (student better)
        loss_student_better = caws_structure_loss(
            teacher_score=0.6, student_score=0.8)

        # Test with student_score < teacher_score (student worse)
        loss_student_worse = caws_structure_loss(
            teacher_score=0.8, student_score=0.6)
        ...
        assert loss_equal.item(
        ) < 1e-5, "student_score == teacher_score should return zero loss with >= check"

        # Student better should return zero loss (no penalty)
        assert isinstance(loss_student_better, torch.Tensor)
        assert loss_student_better.item(
        ) < 1e-5, "student_score > teacher_score should return zero loss"

        # Student worse should return non-zero loss (penalty)
        assert isinstance(loss_student_worse, torch.Tensor)
        assert loss_student_worse.item(
        ) > 0, "student_score < teacher_score should return non-zero loss"
    ...
    def test_caws_structure_loss_if_statement_false_check(self, device):
        ...
        teacher_score = 0.8
        student_score = 0.6
        loss_penalty = caws_structure_loss(
            teacher_score=teacher_score, student_score=student_score)

        # Test with student_score >= teacher_score (should return zero)
        loss_no_penalty = caws_structure_loss(
            teacher_score=0.6, student_score=0.8)
        ...
        assert loss_penalty.item(
        ) > 0, "student_score < teacher_score should return penalty (if statement verified)"
...
class TestEntropyWeighting:
    ...
    def test_entropy_weighting_entropy_calculation_addition_vs_subtraction(self, device):
        ...
        assert not (
            entropy != entropy), "Entropy should not be NaN (addition prevents log(negative))"
    ...
    def test_entropy_weighting_temperature_calculation_subtraction_vs_multiplication(self, device):
        ...
        entropy = weights_dict["entropy"]
        entropy_ratio = (entropy - min_entropy) / (max_entropy -
                                                   min_entropy) if (max_entropy - min_entropy) > 0 else 0.0
    ...
    def test_entropy_weighting_epsilon_addition(self, device):
        ...
        # Very negative logits (tiny probabilities)
...
class TestCombinedKDLossEdgeCases:
    ...
    def test_combined_kd_loss_loss_mask_shape_mismatch_teacher_targets(self, device):
        ...
        loss_mask = torch.ones(batch_size, seq_len + 5,
    ...
    def test_combined_kd_loss_loss_mask_shape_mismatch_ground_truth(self, device):
        ...
        ground_truth_targets = torch.randint(
            0, vocab_size, (batch_size, seq_len), device=device)
        # Create loss_mask with mismatched shape
        loss_mask = torch.ones(batch_size, seq_len - 3,
                               dtype=torch.bool, device=device)
    ...
    def test_combined_kd_loss_with_tool_name_loss(self, device):
        ...
        tool_name_ids = torch.randint(
            0, vocab_size, (batch_size, tool_len), device=device)
        tool_name_mask = torch.ones(
            batch_size, tool_len, dtype=torch.bool, device=device)
    ...
    def test_combined_kd_loss_with_integration_loss(self, device):
        ...
        tool_result_fields = torch.randint(
            0, vocab_size, (batch_size, int_len), device=device)
        integration_mask = torch.ones(
            batch_size, int_len, dtype=torch.bool, device=device)
...
class TestCAWSComplianceLossEdgeCases:
    ...
    def test_caws_compliance_loss_claim_extraction_exception(self, device):
        ...
        mock_extractor = Mock()
        mock_extractor.extract_claims.side_effect = Exception(
            "Extraction failed")

        loss = caws_compliance_loss(
            student_output, teacher_output, claim_extractor=mock_extractor)
...
class TestCodeModePreferenceLossBranchCoverage:
    ...
    def test_code_mode_preference_loss_num_eligible_zero_return_branch(self, device):
        ...
        batch_meta = [
            {"tool_count": 1, "intermediate_sizes": [],
                "pii_tags_present": False},  # Not eligible (min_tools=2)
            {"tool_count": 0, "intermediate_sizes": [],
                "pii_tags_present": False},  # Not eligible
        ]
