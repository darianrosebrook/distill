...
def test_early_tool_ce_only_when_needed():
    ...
    # Mock tokenizer
    tokenizer_stub = Mock()
    tokenizer_stub.convert_tokens_to_ids = Mock(
        side_effect=lambda x: {"{": 5, "[": 6, '"': 7}.get(x, None)
    )
...
def test_early_tool_json_prior_fallback():
    ...
    assert (
        d["early_tool.mean_json_prior_nll0"] != 0.0 or d["early_tool.mean_json_prior_nll0"] == 0.0
    )
