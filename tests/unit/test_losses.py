...
class TestKLDivergence:
    ...
    def test_kl_divergence_temperature(self):
        ...
        assert not torch.isclose(loss_t1, loss_t2), "Different temperatures should give different losses"
    ...
    def test_kl_divergence_identical_distributions(self):
        ...
        assert loss.item() < 1e-5, f"KL divergence should be ~0 for identical distributions, got {loss.item()}"
...
class TestCombinedKDLoss:
    ...
    def test_combined_kd_loss_weights(self):
        ...
        assert not torch.isclose(metrics1["total"], metrics2["total"]), "Different weights should produce different losses"
