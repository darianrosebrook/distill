...
class TestSchemaValidator:
    ...
    def test_set_schema(self):
        ...
        obj1 = {"name": "test"}
        ok, err = validator.validate(obj1)
        assert ok is True
        ...
        # Old object should now fail (missing age)
        ok, err = validator.validate(obj1)
        assert ok is False
        assert "Missing required key: age" in err

        # New object should pass
        obj2 = {"name": "test", "age": 25}
        ok, err = validator.validate(obj2)
        assert ok is True
...
class TestTokenLexicon:
    ...
    def test_candidates_for_chars(self):
        """Test optimized prefix map lookup."""
        tokenizer = MockTokenizer(vocab_size=100)
        lexicon = TokenLexicon(tokenizer)

        # Test lookup for opening brace
        candidates = lexicon.candidates_for_chars(["{", "}"])
        assert 0 in candidates  # token 0 = "{"

        # Test lookup for quote
        candidates = lexicon.candidates_for_chars(['"'])
        assert 2 in candidates  # token 2 = '"'

        # Test lookup for whitespace
        candidates = lexicon.candidates_for_chars([" "])
        assert 11 in candidates  # token 11 = " "

        # Test lookup for multiple chars
        candidates = lexicon.candidates_for_chars(["{", '"', " "])
        assert 0 in candidates
        assert 2 in candidates
        assert 11 in candidates

    def test_by_first_map_built(self):
        """Test that prefix map is built during initialization."""
        tokenizer = MockTokenizer(vocab_size=100)
        lexicon = TokenLexicon(tokenizer)
...
class TestJSONConstrainedDecoder:
    ...
    def test_init(self):
        ...
        assert decoder.base_schema == schema
        ...
        assert decoder.registry is None
        assert decoder._last_applied_tool is None

    def test_init_with_registry(self):
        """Test decoder initialization with registry."""
        schema = {
            "type": "object",
            "required": ["name"],
            "properties": {"name": {"type": "string"}},
...
class TestDecoderIntegration:
    ...
    def test_dynamic_schema_switching(self):
        """Test dynamic schema switching when tool name detected."""
        base_schema = {
            "type": "object",
        ...
        tool_schema = {
            ...
            "required": ["name", "arguments"],
            "properties": {
                "name": {"type": "string", "enum": ["web.search"]},
                "arguments": {
                    "type": "object",
                    "required": ["q"],
        ...
        # Initially should use base schema
        assert decoder.schema == base_schema

        # Simulate generating JSON with tool name
        ...
        # Use a token that doesn't change the buffer much
        state = decoder.push(state, 11)  # space token

        # Schema should have switched
        assert decoder.schema == tool_schema
        assert decoder._last_applied_tool == "web.search"
        mock_registry.get.assert_called_with("web.search")

    def test_start_resets_schema(self):
        """Test that start() resets schema to base schema."""
        base_schema = {
            "type": "object",
            "required": ["name"],
            "properties": {"name": {"type": "string"}},
        ...
        decoder.validator.set_schema(tool_schema)
        decoder.schema = tool_schema
