...
class TestExtractProcessTargets:
    ...
    def mock_tokenizer(self):
        ...
        tokenizer.return_value = {"offset_mapping": [(0, 1), (1, 2), (2, 3)]}
    ...
    def test_extract_process_step_targets_basic(self, mock_tokenizer):
        ...
        targets = extract_process_step_targets(
            teacher_text=teacher_text, tokenizer=mock_tokenizer, tool_names=["read_file"]
        )
    ...
    def test_control_case_filtering(self, mock_tokenizer):
        ...
        assert processed["metadata"].get("integration_spans_bytes", []) == []
        assert (
            processed["metadata"].get("integration_token_ids") is None
            or len(processed["metadata"].get("integration_token_ids", [])) == 0
        )
    ...
    def test_integration_field_extraction(self, mock_tokenizer):
        ...
        teacher_text = (
            "Integration: The file contains 128 lines. Summary: File processed successfully."
        )
        integration_spans = [
            [13, 35],
            [36, 70],
        ]  # "The file contains 128 lines" and "Summary: File processed successfully"
        call_sequence = [{"name": "read_file", "arguments": {"path": "test.txt"}}]
...
class TestVerifyContextualSet:
    ...
    def test_compute_integration_f1(self):
        ...
        integration_spans = [
            [13, 60]
        ]  # Span covering "ANE prefers fp16 kernels when ops are supported."
    ...
    def test_is_long_context_item(self):
        ...
        item_with_flag = {"prompt": "short", "metadata": {"long_context": True}}
    ...
    def test_integration_f1_edge_cases(self):
        ...
        # No matching values
        precision, recall, f1 = compute_integration_f1(
            "Integration: Some text", [[13, 22]], {"summary": "Different text"}
        )
        assert precision == 0.0
        assert recall == 0.0
        ...
        item = {
            ...
            "metadata": {
                ...
                "json_args_spans_bytes": [
                    [8, 8 + len(call1_json)],
                    [20 + len(call1_json), 20 + len(call1_json) + len(call2_json)],
                ],
                "expected_behaviour": "normal",
            },
        }
    ...
    def test_low_integration_f1_flagging(self):
        ...
        if result.get("integration_f1", 1.0) < 0.75:
            assert (
                "low_integration_f1" in result.get("problems", [])
                or result.get("integration_f1", 1.0) < 0.75
            )
    ...
    def test_main_function_schema_validation_errors(self, tmp_path):
        ...
        schema_path.parent.mkdir(parents=True, exist_ok=True)
        with open(schema_path, "w") as f:
            json.dump({"type": "object", "required": ["teacher_text", "metadata"]}, f)
    ...
    def test_multi_call_parity_mismatch(self):
        ...
        assert any("mismatch" in p.lower() for p in problems) or any(
            "expected_2" in p for p in problems
        )
...
class TestErrorHandling:
    ...
    def test_main_function_basic(self, tmp_path):
        ...
        try:
            ...
            with patch(
                "scripts.extract_process_targets.load_tokenizer", return_value=mock_tokenizer
            ):
                main()
        except SystemExit:
            pass
...
class TestNegativeMutationSimulation:
    ...
    def test_control_case_not_removed(self):
        ...
        assert len(result.get("problems", [])) == 0 or "control" not in str(
            result.get("problems", [])
        )
    ...
    def test_token_alignment_boundary(self):
        ...
        result3 = bytes_to_token_span(
            text, len(text.encode("utf-8")) + 10, len(text.encode("utf-8")) + 20, mock_tokenizer
        )
