...
@given(
    text=st.text(min_size=10, max_size=100, alphabet=st.characters(min_codepoint=32, max_codepoint=126)),
...
def test_span_round_trip_ascii(text, start, end, mock_fast_tokenizer):
    ...
    encoded = mock_fast_tokenizer.encode(text_norm, add_special_tokens=False, return_offsets_mapping=True)
...
def test_normalization_stability(text, text_norm, line_endings):
    ...
    normalized_twice = normalize_text_for_alignment(normalized, text_norm=text_norm, line_endings=line_endings)
...
@given(
    text=st.text(min_size=5, max_size=30, alphabet=st.characters(min_codepoint=0x00, max_codepoint=0x10FFFF)),
)
