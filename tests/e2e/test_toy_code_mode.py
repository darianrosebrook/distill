...
def test_toy_code_mode_with_span_targets():
    ...
    class MockTokenizerWithOffsets:
        ...
        def encode_plus(self, text, add_special_tokens=False, return_offsets_mapping=False, return_tensors=None, **kwargs):
    ...
    assert len(span_targets_dict["direct_tool_spans"]) == 0, "TS code should not have direct tool spans"
    ...
    for start_token, end_token in span_targets_dict["ts_mode_spans"]:
        assert 0 <= start_token < num_tokens, f"Start token {start_token} out of range [0, {num_tokens})"
