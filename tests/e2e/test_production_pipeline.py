...
def test_production_pipeline_e2e(temp_dir, checkpoint_path):
    ...
    print(
        f"   Model: d_model={arch_cfg.get('d_model')}, "
    ...
    result = subprocess.run(
        [
            ...
            "prefill",
            "--seq",
            "1024",  # Example sequence length
            "--enumerated-T",
        ]
        + [str(s) for s in enumerated_shapes],
        capture_output=True,
        text=True,
        env=env,
    )
    ...
    has_compat_warning = "torch version" in stderr_lower and "has not been tested" in stderr_lower
    ...
    if has_compat_warning:
        print("⚠️  PyTorch compatibility warning (non-fatal)")
    ...
    if shape_validation:
        ...
        if primary_status != "ok":
            print("⚠️  Primary shape T1024 validation failed")
