...
def default_verifier(
    ent_table=None, coverage_min=0.7, support_min=0.6, contradict_min=0.55, insufficient_min=0.55
) -> CAWSClaimVerification:
...
def test_outcome_3_right_answer_wrong_rationale_insufficient():
    ...
    ent_table = {
        ...
        # NOT Ec ⊨ cmax
        (evid["evidence"][0]["text"], cmax_text): {
            "support": 0.2,
            "contradict": 0.0,
            "insufficient": 0.8,
        },
...
def test_outcome_4_retrieval_mismatch_insufficient():
    ...
    ent_table = {
        ...
        ("Acme launched the product in Berlin on Monday.", cmax_text): {
            "support": 0.8,
            "contradict": 0.0,
            "insufficient": 0.2,
        },
        # Ec ⊨ cmax (weak evidence doesn't support cmax either)
        ("Something happened.", cmax_text): {
            "support": 0.2,
            "contradict": 0.0,
            "insufficient": 0.8,
        },
    ...
    v = CAWSClaimVerification(
        ...
        policy=ClaimsPolicy(
            ...
            thresholds={"coverage_min": 0.7},
            version="test",
        ),
    )
    v.thresholds.update(
        {"coverage_min": 0.8, "support_min": 0.6, "contradict_min": 0.55, "insufficient_min": 0.55}
    )
    v.precedence[:] = ["contradict", "support", "insufficient"]
    res = v.verify_claim_evidence(claim, evid)
...
def test_outcome_5_contradiction_has_precedence_over_support():
    ...
    claim = mk_claim(
        c_text,
        elements=ClaimElements(subject="lights", predicate="were", object="on", qualifiers={}),
    )
...
def test_outcome_6_insufficient_evidence_when_all_below_thresholds():
    ...
    evid = mk_manifest([evid_c["evidence"][0]["text"], evid_cmax["evidence"][0]["text"]])
    ...
    ent_table = {
        (evid_c["evidence"][0]["text"], c_text): {
            "support": 0.4,
            "contradict": 0.0,
            "insufficient": 0.6,
        },
        # insufficient is highest
        (evid_cmax["evidence"][0]["text"], cmax_text): {
            "support": 0.3,
            "contradict": 0.0,
            "insufficient": 0.7,
        },
        # support is highest, above threshold
        (evid_c["evidence"][0]["text"], cmax_text): {
            "support": 0.65,
            "contradict": 0.0,
            "insufficient": 0.35,
        },
    ...
    v.thresholds.update(
        {"coverage_min": 0.7, "support_min": 0.6, "contradict_min": 0.55, "insufficient_min": 0.55}
    )
    v.precedence[:] = ["contradict", "support", "insufficient"]
    res = v.verify_claim_evidence(claim, evid)
...
def test_binding_aware_coverage_requires_spo_same_sentence():
    ...
    claim = mk_claim(
        c_text,
        elements=ClaimElements(subject="Acme", predicate="acquired", object="Beta", qualifiers={}),
    )
    evid = mk_manifest(
        [
...
def test_negation_mismatch_penalizes_coverage_and_blocks_verified():
    ...
    claim = mk_claim(
        c_text,
        elements=ClaimElements(
            subject="Acme", predicate="ship", object="product", qualifiers={"negation": True}
        ),
    )
