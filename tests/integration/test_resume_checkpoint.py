...
class TestCheckpointManager:
    ...
    def test_checkpoint_preserves_results(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Create many results
            results = [{"prompt": f"test{i}", "teacher_text": f"response{i}"} for i in range(100)]
...
class TestResumeWorkflow:
    ...
    def test_resume_skips_completed_indices(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            results = [
                {"prompt": f"test{i}", "teacher_text": f"response{i}"} for i in completed_indices
            ]
            budget_tracker = BudgetTracker()
            manager.save_checkpoint(completed_indices, results, budget_tracker, time.time())
...
class TestCacheIntegration:
    ...
    def test_cache_validation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Save valid cache
            prompt = "test prompt"
            result = {"prompt": prompt, "teacher_text": "response", "metadata": {}}
