    ...
    class MockTokenizer:
        ...
        def decode(self, token_ids, skip_special_tokens=False, **kwargs):
            ...
            if skip_special_tokens:
                token_ids = [t for t in token_ids if t != self.eos_token_id and t != self.pad_token_id]
    ...
    samples = [
        {
            ...
            "tool_name_mask": [1] * len(mock_tokenizer.encode("read_file", add_special_tokens=False)),
            "gold_json_text_ids": mock_tokenizer.encode('{"name": "read_file", "arguments": {"path": "config.json"}}', add_special_tokens=False),
            "mask_valid_json_tokens": [1] * len(mock_tokenizer.encode('{"name": "read_file", "arguments": {"path": "config.json"}}', add_special_tokens=False)),
        },
        {
            ...
            "tool_name_mask": [1] * len(mock_tokenizer.encode("web.search", add_special_tokens=False)),
            "gold_json_text_ids": mock_tokenizer.encode('{"name": "web.search", "arguments": {"q": "Python tutorials"}}', add_special_tokens=False),
            "mask_valid_json_tokens": [1] * len(mock_tokenizer.encode('{"name": "web.search", "arguments": {"q": "Python tutorials"}}', add_special_tokens=False)),
        },
    ]
...
def test_dataset_loads_process_step_targets(temp_process_step_dataset, mock_tokenizer):
    ...
    assert sample["tool_name_mask"].dtype == torch.bool or sample["tool_name_mask"].dtype == torch.int64
    assert sample["mask_valid_json_tokens"].dtype == torch.bool or sample["mask_valid_json_tokens"].dtype == torch.int64
...
def test_training_step_with_process_step_targets(temp_process_step_dataset, mock_tokenizer, device, small_model_cfg):
    ...
    try:
        ...
        # Verify loss dict structure
        assert "total" in loss_dict, "Loss dict should have 'total' key"
        assert "proc_total" in loss_dict or "proc_json_validity" in loss_dict, "Loss dict should have process supervision losses"
