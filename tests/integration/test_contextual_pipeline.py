...
def test_determinism_full_pipeline(mock_tokenizer, tmp_path):
    ...
    result1 = subprocess.run(
        [
            sys.executable, "-m", "scripts.generate_contextual_prompts",
            "--out", str(gen1_out),
    ...
    result2 = subprocess.run(
        [
            sys.executable, "-m", "scripts.extract_process_targets",
            "--in", str(gen1_out),
            "--out", str(ext1_out),
    ...
    result3 = subprocess.run(
        [
            sys.executable, "-m", "scripts.verify_contextual_set",
            "--in", str(ext1_out),
            "--report", str(verify1_out),
        ],
        capture_output=True,
        text=True,
        cwd=Path(__file__).parent.parent.parent,
    ...
    if report1["summary"].get("avg_integration_f1_macro_lax") is not None:
        assert report1["summary"]["avg_integration_f1_macro_lax"] == report2["summary"]["avg_integration_f1_macro_lax"]
