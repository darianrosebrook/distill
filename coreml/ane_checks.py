...
def main():
    ...
    ap.add_argument(
        "--min-ane-pct", type=float, default=0.80, help="Minimum ANE percentage (default: 0.80)"
    )
    args = ap.parse_args()

    # Check for placeholder marker
    mlpackage_path = Path(args.mlpackage)
    is_placeholder = (mlpackage_path.parent / ".placeholder").exists() or (
        mlpackage_path / ".placeholder"
    ).exists()
    ...
    try:
        ...
        if args.ane_required:
            ...
            print("[ane_checks] Note: Actual ANE residency measurement requires runtime profiling")
            print("[ane_checks] Use coreml/runtime/ane_monitor.py for detailed residency analysis")
