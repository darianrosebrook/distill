...
class PromptCache:
    ...
    def _cache_size(self) -> int:
        ...
        for entry in self.cache.values():
            ...
            # Estimate size: sum of numpy array sizes
            for key, value in state.items():
                if isinstance(value, np.ndarray):
                    total += value.nbytes
                elif isinstance(value, dict):
                    # Recursively estimate dict size (simplified)
                    total += sum(v.nbytes if isinstance(v, np.ndarray) else 0 for v in value.values())
        return total
