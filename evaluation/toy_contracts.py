...
def micro_f1(samples: List[Dict[str, str]]) -> float:
    ...
    for s in samples:
        ...
        tool_tokens_present = any(tok in pred for tok in ["tool", "call", "{", "ok"])
...
def greedy_decode_sequence(logits: np.ndarray, id2tok: Dict[int, str], max_steps: int = 16) -> str:
    ...
    toks = [greedy_decode_logits(arr[t], id2tok) for t in range(min(arr.shape[0], max_steps))]
    return " ".join(toks)
...
def main():
    ...
    ap.add_argument(
        "--model", required=True, help="Path to a single mlpackage OR one of the enumerated shapes"
    )
    ap.add_argument("--model-dir", help="Directory containing shape-specific models (optional)")
    ...
    ap.add_argument("--id2tok", help="Optional JSON {id:str} mapping for decoding")
    args = ap.parse_args()
    ...
    targets = [{"target": "ok tool.call{...}"}, {"target": "ok"}]
    ...
    for L in args.seq:
        ...
        try:
            ...
            per_shape[str(L)] = {"ok": False, "reason": f"predict error: {type(e).__name__}"}
            continue
        ...
        if logits is None:
            per_shape[str(L)] = {"ok": False, "reason": "no logits-like output"}
            continue

        arr = np.array(logits)
        nan_here, zero_here = has_nan_or_zero(arr.ravel()[: min(arr.size, 2048)])
    ...
    gates_ok = (compiled >= 1) and (total_nan == 0) and (total_zero == 0) and (f1 >= 0.2)
