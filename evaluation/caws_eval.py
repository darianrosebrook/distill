...
def validate_provenance_clarity(rationale: Optional[str], evidence_manifest: Optional[Dict],
    ...
    evidence_present = evidence_manifest is not None and len(
        evidence_manifest.get("evidence_items", [])) > 0
    diff_present = change_diff is not None and len(change_diff.strip()) > 0

    # Simple alignment score: all components present = 1.0
    alignment_score = 1.0 if (
        rationale_present and evidence_present and diff_present) else 0.0
...
def evaluate_caws_compliance(change_id: str, working_spec: Dict,
    ...
    budget_adherence = validate_budget_adherence(
        change_diff, max_loc, max_files)
    gate_integrity = validate_gate_integrity(
        test_results, lint_results, coverage_results)
    provenance_clarity = validate_provenance_clarity(
        rationale, evidence_manifest, change_diff)
...
def _load_working_spec(spec_path: str) -> Dict:
    ...
    if yaml is None:
        raise ImportError(
            "PyYAML required for CAWS evaluation. Install with: pip install pyyaml")
...
def _run_linter() -> Dict:
    ...
    for linter_name, cmd in linters:
        try:
            ...
            return {
                "no_errors": False,
                "error": f"{linter_name} timed out"
            }

    # No linter found - assume passing (user may not have linter configured)
    return {
        "no_errors": True,
        "warning": "No linter found, assuming no errors"
    }
...
def main(working_spec: str = typer.Argument(...),
    ...
    evidence_data = _load_json_file(
        evidence_manifest) if evidence_manifest else None
    ...
    if test_results:
        ...
        test_data = {"all_passed": False,
                     "warning": "Test results not provided"}
    ...
    if coverage_results:
        ...
        coverage_data = {"meets_threshold": True,
                         "warning": "Coverage results not provided"}
