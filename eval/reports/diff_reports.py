...
def diff_reports(report_a_path: str, report_b_path: str, threshold: float = 0.01) -> Dict[str, Any]:
    ...
    for key in fields:
        ...
        if key in ["avg_integration_f1_macro_lax", "avg_integration_f1_macro_strict", 
                   "integration_f1_eligible_count", "privacy_ok_rate", "integration_coverage"]:
            if delta < -threshold:
                regressions.append({
                    "metric": key,
                    "baseline": val_a,
...
def main():
    ...
    if result["regressions"]:
        ...
        for r in result["regressions"]:
            print(f"  - {r['metric']}: {r['baseline']:.4f} → {r['current']:.4f} (Δ {r['delta']:.4f})", file=sys.stderr)
        sys.exit(2)
